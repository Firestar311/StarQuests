 ...
 */
public class BucketEmptyAction extends EventAmountAction<Material, PlayerBucketEmptyEvent> {
     ...
     */
    public BucketEmptyAction(Material material, int amount) {
        super("bucketempty", material, amount);
        if (!ACCEPTED_MATERIALS.contains(material)) {
            throw new RuntimeException("Invalid material for bucketempty action " + material);
        }
    }
     ...
     * @param amount    The amount of materials
     */
    public BucketEmptyAction(List<Material> materials, int amount) {
        super("bucketempty", materials, amount);
    ...
    @Override
    protected void handleEvent(PlayerBucketEmptyEvent event, Player player, Quest quest, QuestObjective questObjective, StorageHandler storageHandler, AmountQuestData questData) {
